...
|---------|--------|------|------------|------|
| **部署层**（token、URL、端口、风控） | 各仓 `<仓>/.env`（= `deploy/repos/<svc>/.env`） | 你 | `docker compose up -d` 重建容器 | TUSHARE_TOKEN、QMT_BRIDGE_BASE_URL |
...
docker compose up -d vortex-data            # 先只起数据服务，最稳
# 全栈（data+qmt+backtest）： docker compose up -d
...
改了 `deploy/repos/<svc>/.env` 任何值后，重跑同一条 `up -d` 即可，compose 检测到变化会重建容器：

```bash
cd vortex_common/deploy
docker compose up -d vortex-data
```
...
主机侧端口/绑定不在各仓 `.env`，而是 `up` 时用 shell env 覆盖（默认只绑回环、最安全）。
**先确保设了对应服务的写 token**，再：
...
| 现象 | 原因 | 解决 |
|------|------|------|
| `up -d` 报 env file `repos/.../.env` not found | 没先 `./pull-code.sh`，或该仓没 `.env.example` | 先 pull-code；缺模板的仓按迁移指南 §5 补 |
| 改了 `.env` 没效果 | 没在 `deploy/` 里跑，或没重新 `up -d` | `cd deploy` 后重跑 `up -d <svc>` |
| 抓数一直 blocked_credentials | `TUSHARE_TOKEN` 没填 | 填进 `repos/vortex_data/.env` 后 `up -d` |
| 写操作 401 | 没设对应服务的写 token | 设 token 后 `up -d`，请求带 token |
...
docker compose up -d vortex-data     # 起/重建数据服务（或不带名=全栈）
